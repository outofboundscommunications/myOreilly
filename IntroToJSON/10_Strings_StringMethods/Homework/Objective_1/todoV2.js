...
// array to hold the instances of the search results
// we store the object id in this array of the objects that contained
// a 'hit' or positive result of the user's search query
...
function searchTodos()	{
	var searchTerm = document.getElementById("searchString").value;
	// clean up string of any blanks
	searchTerm = searchTerm.trim();
	// make sure search field is not blank
	if (checkInputText(searchTerm,"Please enter a search string")) return;
	// search for the string in the array of the todos by looping thru the array
	// looking for the string within either the 'task' property or the 'who' property
	...
	for (var i= 0; i <todos.length; i++)	{
		...
		var currentObjectToSearch = todos[i].who + todos[i].task;
		console.log('this is the current object:' + currentObjectToSearch);
		// convert the object to a json string so we can search it
		var textToSearchJSON = JSON.stringify(currentObjectToSearch);
		console.log('the json text of this object is: ' + textToSearchJSON);
		// now use another loop so we find all instances of the searchTerm that appear in the textToSearchJSON
		var pos = textToSearchJSON.toUpperCase().indexOf(searchTerm.toUpperCase(),pos);
		// if the pos value is >0, that means we have a 'hit'
		if (pos >=0)	{
		  idOfObject = todos[i].id;
		  console.log('the id of the oject with the hit is: ' + idOfObject);
		  // store the object that contained a 'hit' in the array of searchResults
		  searchResults.push(idOfObject);
		  }
	}
	if (searchResults.length !=0)	{
		console.log('the search found the following object id: ' + searchResults);
		showResults();
	}
	else	{
...
function showResults() {
	/*var ul = document.getElementById("mySearchResults");
	var frag = document.createDocumentFragment();*/
	//look up the ID of the object, that were 'hits' in the search, in the todos array and display to screen
		// loop thru the todos array looking for the object ID stored in the searchResults array
		// match the ID and display
		for (var i = 0; i < searchResults.length; i++) {
			for (j = 0; j < todos.length; i++)	{
				console.log('the id of the current search results item is: ' + searchResults[i]);
				console.log('the id of the current object in the todo list is: ' + todos[j].id);
				/*if (todos[j].id == searchResults[i])	{
					console.log("Found an object in the array: " + todos[j].id + todos[j].task + todos[j].who + todos[j].dueDate);
				}
				else	{
					console.log('no hit');
				}*/
