...
function searchTodos()	{
	// clear the results from prior search from screen
	/*var priorSearchResults = document.getElementById("mySearchResults");
	console.log(priorSearchResults.childNodes);
	priorSearchResults.parentNode.removeChild(priorSearchResults);*/
	...
	//empty the searchResults array from prior results
	searchResults = [];
	...
	for (var i= 0; i <todos.length; i++)	{
		...
		if (pos >=0)	{
		  console.log('the id of the oject with the hit is: ' + todos[i].id);
		  // store the id of the todo object that contained a 'hit' into the array of searchResults
		  searchResults.push(todos[i].id);
		  }
	}
...
function showResults() {
	// create ul element for containing search results list
	...
	for (i = 0; i < searchResults.length; i++)	{
		for (j = 0; j < localStorage.length; j++)	{
			...
			console.log(key);
			var pos = key.indexOf(searchResults[i],pos);
			if (pos > 0)	{
				console.log('match found');
				var item = localStorage.getItem(key);
				//parse the todo item back into an object for display on page
				var todoItem = JSON.parse(item);
				var li = document.createElement("li");
