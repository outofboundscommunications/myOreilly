...
// from the user submitted form and from the local storage
// this array is used to hold all the color values 
// and is called by the function addColorsToPage()
// that displays all the colors
...
function getFormData() {
	...
	for (i=1;i<=5; i++)	{
		...
		if (checkInputText(colorValue, "Please enter a color in field: "+ [i])) return;
		...
		// use the maxColorId color counter as the key
		// we save the color object to local storage
		var key = "color"+maxColorId;
		// add the key/value pair to local storage
		addColorItem(key,colorValue);
		//and increment the global counter of how many colors
		//there are by one
		maxColorId++;
	}
...
function addColorItem(key,value) {
